...
- Editing plan frontmatter or sidecar YAML directly — **`plan-state.mjs`** is the sole writer for sidecar operations-plan fields (`status`, `archived`, `parent`, `worktrees`, `prs`, `session`) per **`.sedea/centers/sedea/rules/8_plan-board-contract.mdc`**; step 3.5 only edits **`## Follow-ups`** markdown bodies.
